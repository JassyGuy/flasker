...
root = Tk()
root.title('Benny Choe')
#root.iconbitmap('c:/mypjt.myico.ico')
...
def add_customer():
    ...
    values = (first_name_box.get(), last_name_box.get(), zipcode_box.get(), price_paid_box.get(), address1_box.get(), address2_box.get(), city_box.get(), state_box.get(), country_box.get(), phone_box.get(), email_box.get(), payment_method_box.get(), discount_code_box.get())
...
def list_customers():
    ...
    #list_customer_query.iconbitmap('c:mypjt')
...
state_labe = Label(root, text="Statel").grid(row=6, column=0, sticky=W, padx=10)
